...
The autocmd group is then assigned to the **augroup** variable, so that adding new autocmds which you can use later to add new autocmds to the group.
...
When you open a file of one of the types listed, autocmd runs the defined callback function. Within this function, it performs two operations:
...
When the *TextYankPost* event occurs, the autocmd executes the defined callback function. Within this function, it calls the `vim.highlight.on_yank()` function, which activates the highlighting of the copied text.
...
This autocmd triggers when you open a file of type **help**, which is the file type used for help pages in Neovim.

When you open a file of the *help* type, the autocmd runs the defined *callback* function. It performs three operation within this function:
...
Vertical help allows you to work more efficiently and productively, as you can easily view help information and navigate between windows without having to leave the current window.
